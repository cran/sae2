eblupDyn <- function(formula, D, TI, vardir, method = c("REML", "ML"),
    ...
    if (inherits(vardir, "list")) {
        ...
        lapply(vardir, FUN = function(x) { 
            if (!all(dim(x) == c(NV*nt, NV*nt))) 
                stop(paste("each element of 'vardir' must be a square matrix",
                           "with 'NV*nt' rows, where nt=", nt))
        })
        vardir.temp <- matrix(0, nrow=D*NV*nt, ncol=D*NV*nt)
        for (d in 1:D) {
            vardir.temp[((d-1)*NV*nt+1):(d*NV*nt), ((d-1)*NV*nt+1):(d*NV*nt)] <-
                vardir[[d]]
        }
        vardir <- vardir.temp
    } else {
        ...
        if (!all(dim(vardir) == c(D*NV*nt, D*NV*nt)))
            stop("'vardir' must be a square matrix with 'D*NV*nt' rows, where nt=", nt)
    }
    if (NV == 1) {
        if (!missing(data)) {
        ...
        nformula <- nrow(X)
        if(nformula != D*nt) 
            stop(paste("length of variables must be D * nt, where nt=", nt))
        y <- formuladata[, 1]
        ...
        y <- rep(0, D*nt*NV)
        X <- matrix(0, nrow=D*nt*NV, ncol=sum(ncolx))
        ...
        for (nv in 1:NV) {
            ...
            if(nformula != D*nt) 
                stop(paste("length of variables must be D * nt, where nt=", nt))
            for (m in 1:D) {
                X[((m-1)*nt*NV+(nv-1)*nt+1):((m-1)*nt*NV+nv*nt), cstart:cend] <- 
                    X1[((m-1)*nt+1):(m*nt), ]
                y[((m-1)*nt*NV+(nv-1)*nt+1):((m-1)*nt*NV+nv*nt)] <-      
                    y1[((m-1)*nt+1):(m*nt)] 
            }
            cstart <- cend + 1
        }
